...
class ItemEstimates extends React.Component {
    ...
    render() {
        ...
        var itemList = this
            ...
            .map(function (item, index) {
                ...
                return <tr key={index}>
                    <td>{item.estimateId ? item.estimateId.estimateNumber : '-'}</td>
                    <td>{item.createdAt ? moment(item.createdAt).format('MM/DD/YYYY') : '-'}</td>
                    <td>{item.estimateId ? item.estimateId.title : '-'}</td>
...
function mapStateToProps(state, ownProps) {
    return { itemEstimateList: state.itemCreation.itemEstimateList };
}
...
export default connect(mapStateToProps, mapDispatchToProps)(ItemEstimates);
