...
import { taxRateMask, markUpMask} from '../../constants/customMasks';
...
class EstimateEdit extends React.Component {
	...
	componentWillMount() {
		...
		intervalId = setInterval(this.sendEditStatus, 3000);
		...
		// await 
		Promise.all([this.props.actions.getEstimateDetails(estimateId), this.props.actions.getRevisionList(estimateId)]);
	...
	componentWillReceiveProps(nextProps) {
		...
		let item = [];
		// if(nextProps.updatedEstimate.length!=0){
		//     
		// }
		...
		if (nextProps.companyList.length == 0 && nextProps.individualList.length == 0 && nextProps.salesRepList.length == 0) {
			if (nextProps.estimateDetail.length != 0) {
				...
				let opportunityObj = {
					...
					label: estimatestate.opportunityId ? estimatestate.opportunityId.opportunityNumber : '',
				...
				//below code binds value of proposed services not as per revision
				...
				//     });
				// }
				// proposedServices=estimatestate.proposedServices;
				...
				this.setState({
					...
					pvalue: estimatestate.proposedServices?estimatestate.proposedServices:''
				});
			}
		}
	...
	handleOpportunityChange(e) {
		...
		if (opportunityId !== '' && opportunityId != '0') {
			...
			ReactDOM.findDOMNode(this.refs.name).value = currentData.opportunityNumber;
			let data = {
	...
	handleGlobalMarkup(e) {
		...
		// if ((markup <= 100) && this.state.itemPricedData.length !== 0) {
	...
	onChangeMarkupCost(index, dataType, e) {
		...
		if (dataType == 'MARKUP') {
			...
			currentRowItem.mExtended = currentRowItem.quantity * currentRowItem.mCost;//added later
			currentRowData.splice(index, 1, currentRowItem);
			this.updateRevisionItems(currentRowData);//added later
	...
	sendEditStatus() {
		...
		socket.emit('edit in progress', modifierName);
	}
	updateEstimateHandler() {
		if (jQuery('#createEstimate').valid()) {
			...
			this
				.props
				.actions
				.updateEstimate(estimateData, this.props.params.estimateId);
			setTimeout(function () {
				$('div#update_estimate').unblock();
			}, 200);
			// }
			...
			.map(function (i, index) {
				...
				if (i.itemTypeId === 1) {
					...
					return (
						<tr key={index} data-id={index}>
							<td><span className="handle"><i className="fa fa-bars"></i></span></td>
							<td className="selectable-header" colSpan="9" style={{  color: '#3598dc' }}>
				...
				if (i.itemTypeId === 1) {
					return (<tr key={index} data-id={index}>
						...
						<td style={{ textAlign: 'right', color: '#3598dc' }}><RIEInput
							...
							format={this.formatInteger}
							value={i.mMarkup}
							// value={i.mMarkup ? (i.mMarkup.toString()) : 0}
							propName="data"
					...
					return (
						<tr key={index} data-id={index}>
							<td><span className="handle"><i className="fa fa-bars"></i></span></td>
							<td className="selectable-header" colSpan="13" style={{  color: '#3598dc' }}>
								<RIEInput activeClassName="form-control input-xlarge"
									classEditing="form-control"
									value={i.headerName ? i.headerName : 'NA'} propName="data"
									change={this.updateDesc.bind(this, index, 'SUB_HEAD')}
			...
			.map(function (i, index) {
				...
				if (i.itemTypeId === 1) {
					...
					return (
						<tr key={index} data-id={index}>
							<td><span className="handle"><i className="fa fa-bars"></i></span></td>
							<td className="selectable-header" colSpan="11" style={{ color: '#3598dc' }}>
								<RIEInput activeClassName="form-control input-xlarge"
									classEditing="form-control"
									value={i.headerName ? i.headerName : 'NA'} propName="data"
									change={this.updateDesc.bind(this, index, 'SUB_HEAD')}
						...
						<ul className="nav nav-tabs">
							...
							<div className="text-right" >
								...
								<Link to={'/estimate/' + this.props.params.estimateId} className="btn red">
									Cancel </Link>&nbsp;&nbsp;
								<button type="button" className="btn blue" onClick={this.updateEstimateHandler}>Save</button>
							</div>
						</ul>
					</div>
...
export default connect(mapStateToProps, mapDispatchToProps)(EstimateEdit);
