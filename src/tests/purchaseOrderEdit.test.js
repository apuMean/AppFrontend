...
describe('Update Purchase Order Controlled component', () => {
    it('Should have active tab with text purchase order', function (done) {
        const wrapper = mount(<Provider store={store}><POEdit {...props} /></Provider>);
    ...
    it('Should have class portlet-title for all title', function (done) {
        const wrapper = mount(<Provider store={store}><POEdit {...props} /></Provider>);
        expect(wrapper.find('div.portlet-title').length).toBe(8);
        done();
    });
    ...
    it('Purchase order save button should be of type button', (done) => {
        const wrapper = mount(<Provider store={store}><POEdit {...props} /></Provider>);
